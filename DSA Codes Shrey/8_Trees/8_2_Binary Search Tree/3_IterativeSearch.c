// C code for iterative Search operation in BST
...
int searchBST(struct node *root, int key)  // iterative search
{
    while (root != NULL)
    {
        if (key < root->data)
...
{
    int numNodes;
    printf("C Code for Iterative search operation in BST \n");
